...
public class D11_ReporterList {
	...
	public static void main(String[] args) {
		...
		// 1) 어떤 이메일 도메인(yna.co.kr, hankyung.com 등)이 
		//    몇 개씩 있는지 세어서 출력해보기
		D11_ReporterList list = new D11_ReporterList();
		String[] lines = list.reporters.split("\n");
		...
		for (String domain : domainCount.keySet()) {
			int count = domainCount.get(domain);
			System.out.println(domain + ": " + count + "개");
		}
		System.out.println("------------------------------");
		
		// 2) 어떤 신문사의 기자가 몇 명씩 있는지 세어서 출력해보기
		HashMap<String, Integer> reportersCount = new HashMap<>();
		
		for (String line : lines) {
			String newspaper = line.split(" ")[0];
			reportersCount.put(newspaper, reportersCount.getOrDefault(newspaper, 0) + 1);
		}
		
		for (String newspaper : reportersCount.keySet()) {
			int reporterCount = reportersCount.get(newspaper);
			System.out.println(newspaper + ": " + reporterCount + "명");
		}
		System.out.println("------------------------------");
		
		// 3) 어떤 분야의 기자가 몇 명씩 있는지 세어서 출력해보기
		HashMap<String, Integer> fieldsCount = new HashMap<>();
		...
		for (String line : lines) {
			...
			if (fields.length >= 4) {
				...
				fieldsCount.put(field, fieldsCount.getOrDefault(field, 0) + 1);
			}
		}
		
		for (String field : fieldsCount.keySet()) {
			int reporterCount = fieldsCount.get(field);
			System.out.println(field + ": " + reporterCount + "명");
		}
		
		// 4) 모든 기자의 이름/이메일/분야를 신문사별로 분류하여 출력해주기
		HashMap<String, List<String>> categoryCompany = new HashMap<>();
		for (String line : lines) {
			String[] fields = line.split(" ");
			
			if (fields.length >= 4) {
				String newspaper = fields[0];
                String field = fields[1];
                String name = fields[2];
                String email = fields[3];
                
                String reporterInfo = name + "/" + email + "/" + field;
                categoryCompany.putIfAbsent(newspaper, new ArrayList<>());
                categoryCompany.get(newspaper).add(reporterInfo);
			}
		}
	}
}
